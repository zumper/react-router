...
const compile = (options) => {
  ...
  if (path && patterns) {
    ...
    const keys = []
    const keyRe = new RegExp(':(' + Object.keys(patterns).join('|') + '\\b)([*?]?)', 'g')
    const newPath = [
      '^',
      ...
      path.replace(keyRe, (_, name, op) => {
        ...
        if (warn) {
          if (WARN_RE.test(source)) {
            console.error('dangerous pattern detected (nested groups)', name, source)
          }
          if (pattern.flags && pattern.flags !== flags) {
            console.error('sub pattern "%s" has flags "%s", and they contradict global flags: "%s"',
                          name, pattern.flags, flags)
          }
        }
  ...
  if (warn) {
    console.error('nothing found to compile: returning options unchanged', options)
  }
  return options
}
