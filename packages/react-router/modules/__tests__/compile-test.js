...
describe('compile', () => {
  ...
  describe('with path="/somewhere/else"', () => {
    ...
    describe('+strict compiles to `^/somewhere/else/?`', () => {
      const compiled = compile({ path: '/somewhere/else', patterns: {}, strict: true })
    ...
    describe('+exact compiles to `^/somewhere/else$`', () => {
      const compiled = compile({ path: '/somewhere/else', patterns: {}, exact: true })
  ...
  describe('with path="/somewhere/:variable/:other"', () => {
    ...
    describe('compiles to `^/somewhere/([a-z]+)/(?:._.)*happy)`', () => {
      const compiled = compile({ path: '/somewhere/:variable/:other', patterns, })
      ...
      it('produces keys `[variable, other]`', () => {
        expect(compiled.keys).toEqual([{name: 'variable', optional: false},
  ...
  describe('with path="/(?:some)?\\where/:variable(?:/ending)?"', () => {
    ...
    describe('compiles to `^/(?:some)?\\where/([a-z]+)(?:/ending/?)?$`', () => {
      const compiled = compile({ path: '/(?:some)?\\where/:variable(?:/ending/?)?', patterns, exact: true, strict: true})
      it('compiles to `^/(?:some)?\\where/([a-z]+)(?:/ending/?)?$`', () => {
        expect(compiled.re.source).toBe(/^\/(?:some)?\where\/([a-z]+)(?:\/ending\/?)?$/.source)
      })
  ...
  describe('with path="/:one/mid/:two?"', () => {
    ...
    describe('compiles to `^/([a-z]+)/mid/(\\d+)`', () => {
      ...
      it('produces keys `[one, two:optional]`', () => {
        expect(compiled.keys).toEqual([{name: 'one', optional: false},
